...
public:
    ...
    typedef std::function<bool(const std::string&)> tensor_filter_t;
    bool load_tensors(on_new_tensor_cb_t on_new_tensor_cb,
                      int n_threads = 0,
                      bool use_mmap = false,
                      tensor_filter_t filter_func = nullptr);
    bool load_tensors(std::map<std::string, struct ggml_tensor*>& tensors,
                      std::set<std::string> ignore_tensors = {},
                      int n_threads                        = 0,
                      bool use_mmap                        = false);
    bool load_tensors_subset(std::map<std::string, struct ggml_tensor*>& target_tensors,
                      int n_threads,
                      bool enable_mmap);
